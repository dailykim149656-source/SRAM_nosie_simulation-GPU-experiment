"""Torch-accelerated runtime kernels shared by native backend fallbacks."""
...
from backends.torch_portable import resolve_torch_accelerator


def _resolve_runtime() -> Optional[Tuple[str, str, str]]:
    try:
        return resolve_torch_accelerator("gpu")
    except RuntimeError:
        return None
...
def simulate_array_torch_accelerated(request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    runtime = _resolve_runtime()
    if runtime is None or torch is None:
        return None

    device, accelerator_backend, device_label = runtime
    dtype = torch.float64
...
def predict_lifetime_torch_accelerated(request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    runtime = _resolve_runtime()
    if runtime is None or torch is None:
        return None
...
def optimize_design_torch_accelerated(request: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
    runtime = _resolve_runtime()
    if runtime is None or torch is None:
        return None
